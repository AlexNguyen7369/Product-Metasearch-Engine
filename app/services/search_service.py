...
Filtering/sorting is applied here, after the cache lookup, rather than
baked into the cache key: each query text caches one superset of results,
and different filter/sort combinations are cheap to recompute in-process
on every request instead of fragmenting the cache per combination.
"""
...
async def search_products(
    ...
    return SearchResponse(query=query, cached=was_cached, results=products)
